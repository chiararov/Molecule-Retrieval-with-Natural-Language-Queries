...
import pandas as pd
import numpy as np	
from tools import select_random_file
from itertools import combinations

class GraphTextDataset(Dataset):
    def __init__(self, root, gt, split, tokenizer=None, transform=None, pre_transform=None, if_crossmodal=False):
        self.root = root
        self.gt = gt
        self.split = split
        ...
        self.if_crossmodal = if_crossmodal
    ...
    def processed_dir(self) -> str:
        return osp.join(self.root, 'processed_bis/', self.split)
    ...
    def process_graph(self, raw_path):
      ...
      with open(raw_path, 'r') as f:
        ...
        for line in f: #get mol2vec features:
          ...
          tokens.append(substruct_id)
          if substruct_id in self.gt.keys():
            x.append(self.gt[substruct_id])
          else:
            x.append(self.gt['UNK'])
        return torch.LongTensor(edge_index).T, torch.FloatTensor(np.array(x)), tokens
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            edge_index, x, tokens = self.process_graph(raw_path)
            data = Data(tokens=tokens, x=x, edge_index=edge_index, input_ids=text_input['input_ids'], attention_mask=text_input['attention_mask'])
    ...
    def get(self, idx):
        ...
        if self.if_crossmodal:
            another_data = torch.load(select_random_file(self.processed_dir))
            data.text_input = another_data.text_input
        return (data, self.idx_to_cid[idx])
...
class GraphDataset(Dataset):
      ...
      with open(raw_path, 'r') as f:
        ...
        for line in f:
          ...
          if substruct_id in self.gt.keys():
            ...
            x.append(self.gt['UNK'])
        return torch.LongTensor(edge_index).T, torch.FloatTensor(np.array(x)), tokens # peut-être que le np.array(x) est inutile
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            cid = int(raw_path.split('/')[-1][:-6])
            edge_index, x, tokens = self.process_graph(raw_path)
            data = Data(tokens=tokens, x=x, edge_index=edge_index)
            torch.save(data, osp.join(self.processed_dir, 'data_{}.pt'.format(cid)))
            i += 1
